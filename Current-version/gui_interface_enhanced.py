...
class ScannerGUI:
    ...
    def __init__(self, root):
        ...
        # Auto-scan stability controls: require repeat confirmations before accepting shaky reads.
        self.stability_timeout = 8.0
        self.repeat_accept_cooldown = 12.0
        self._stability_key = None
        self._stability_count = 0
        self._stability_last_seen = 0.0
        self._last_accepted_key = None
    ...
    def _setup_qr_tab(self, parent):
        ...
        # Convert qrcode image to PIL Image and resize
        try:
            # qrcode returns PyPNGImage, which is compatible with PIL
            # Suppress type warnings - runtime compatibility verified
            if hasattr(discord_img, 'convert'):
                discord_img = discord_img.convert("RGB")  # type: ignore
            if hasattr(discord_img, 'resize'):
                discord_img = discord_img.resize((200, 200), Image.Resampling.LANCZOS)  # type: ignore
        except Exception:
            pass
        
        try:
            discord_photo = ImageTk.PhotoImage(discord_img)  # type: ignore
            qr_label = tk.Label(discord_frame, image=discord_photo, bg='#2b2b2b')
            qr_label.pack(pady=10)
            # Store reference in dictionary to prevent garbage collection
            if not hasattr(self, '_photo_references'):
                self._photo_references = []
            self._photo_references.append(discord_photo)
        except Exception:
            pass
        qr_label.pack(pady=10)
        ...
        try:
            # qrcode returns PyPNGImage, which is compatible with PIL
            # Suppress type warnings - runtime compatibility verified
            if hasattr(website_img, 'convert'):
                website_img = website_img.convert("RGB")  # type: ignore
            if hasattr(website_img, 'resize'):
                website_img = website_img.resize((200, 200), Image.Resampling.LANCZOS)  # type: ignore
        except Exception:
            pass
        
        try:
            website_photo = ImageTk.PhotoImage(website_img)  # type: ignore
            qr_label2 = tk.Label(website_frame, image=website_photo, bg='#2b2b2b')
            # Store reference in dictionary to prevent garbage collection
            if not hasattr(self, '_photo_references'):
                self._photo_references = []
            self._photo_references.append(website_photo)
    ...
    def _init_scanner(self):
        ...
        try:
            ...
            # Use database from Current-Version directory
            db_path = os.path.join(os.path.dirname(__file__), 'unified_card_database.db')
    ...
    def connect_arduino(self):
        ...
        try:
            ser = getattr(self.scanner, 'ser', None)
            if ser:
                try:
                    ser.close()
                except Exception:
                    pass
                setattr(self.scanner, 'ser', None)
        except Exception:
            pass
        ...
        ok = False
        try:
            if getattr(self, 'arduino_plugin', None) and self.arduino_plugin:
                try:
    ...
    def send_arduino_command(self, cmd):
        ...
        if ((getattr(self, 'arduino_plugin', None) and self.arduino_plugin and self.arduino_plugin.is_connected()) or (self.scanner and getattr(self.scanner, 'ser', None))):
    ...
    def _send_arduino(self, cmd):
        ...
        try:
            if getattr(self, 'arduino_plugin', None) and self.arduino_plugin and self.arduino_plugin.is_connected():
                try:
    ...
    def toggle_arduino_monitoring(self):
        ...
        if self.monitor_var.get():
            if not (self.scanner and getattr(self.scanner, 'ser', None)):
                ...
                if not ((getattr(self, 'arduino_plugin', None) and self.arduino_plugin and self.arduino_plugin.is_connected())):
    ...
    def start_camera(self):
        ...
        if getattr(self, 'camera_plugin', None) and self.camera_plugin:
            try:
    ...
    def stop_camera(self):
        ...
        if getattr(self, 'camera_plugin', None) and self.camera_plugin:
            ...
            if getattr(self, 'camera', None) and self.camera:
                try:
                    self.camera.release()
                except Exception:
                    pass
    ...
    def _video_loop(self):
        ...
        while self.running:
            ...
            if card_approx is not None:
                ...
                if self.auto_scan and (time.time() - self.last_scan_time) > self.scan_cooldown and not getattr(self, '_scan_in_progress', False):
                    ...
                    try:
                        ...
                        if frame is not None and card_approx is not None:
            ...
            try:
                last_frame = getattr(self, '_last_display_frame', None)
                smooth_alpha = getattr(self, 'display_smooth_alpha', 0)
                if last_frame is not None and smooth_alpha > 0:
                    try:
                        prev = last_frame.astype('float32')
    ...
    def _card_stability_key(self, card_info):
        """Build a stable identity key for repeated scan confirmations."""
        if not card_info:
            return None
        pid = str(card_info.get('product_id') or card_info.get('UniqueID') or '').strip()
        if pid:
            return f"pid:{pid}"
        name = str(card_info.get('name') or '').strip().lower()
        set_code = str(card_info.get('set') or '').strip().lower()
        number = str(card_info.get('number') or '').strip().lower()
        if not name:
            return None
        return f"name:{name}|set:{set_code}|num:{number}"

    def _required_confirmations(self, card_info):
        """Return how many matching auto scans are needed before accepting a result."""
        reason = str(card_info.get('fallback_reason') or '').strip().lower()
        manual_review = bool(card_info.get('manual_review_required'))

        # pHash relaxed fallback is the noisiest path in live camera mode.
        if reason == 'no_match_at_strict_threshold':
            return 3
        if manual_review:
            return 3
        # Even OCR paths should be seen twice to avoid one-frame glitches.
        return 2

    def _passes_stability_gate(self, card_info, scan_origin='auto'):
        """Gate auto scans until the same candidate is seen enough times."""
        if scan_origin != 'auto':
            return True, 1, 1

        key = self._card_stability_key(card_info)
        if not key:
            return False, 0, 2

        now = time.time()
        required = self._required_confirmations(card_info)

        # Reset if candidate stream went stale.
        if (now - float(getattr(self, '_stability_last_seen', 0.0) or 0.0)) > float(self.stability_timeout):
            self._stability_key = None
            self._stability_count = 0

        if key == self._stability_key:
            self._stability_count += 1
        else:
            self._stability_key = key
            self._stability_count = 1
        self._stability_last_seen = now

        # Prevent repeated saves while the same card remains under camera.
        if key == self._last_accepted_key and (now - float(self._last_accepted_time)) < float(self.repeat_accept_cooldown):
            return False, self._stability_count, required

        if self._stability_count >= required:
            self._last_accepted_key = key
            self._last_accepted_time = now
            return True, self._stability_count, required
    ...
    def _run_scan_thread(self, frame, card_approx):
        ...
        try:
            try:
                self._perform_scan(frame, card_approx, scan_origin='auto')
            except Exception as e:
    ...
    def manual_scan(self):
        """Manual scan"""
        if not self.running or self.current_frame is None or not self.scanner:
        ...
        try:
            ...
            card_approx = None
        if card_approx is not None:
            self._perform_scan(self.current_frame, card_approx, scan_origin='manual')
        else:
            self.log_status("✗ No card detected", error=True)
    
    def _perform_scan(self, frame, card_approx, scan_origin='auto'):
        """Scan card"""
        if frame is None or card_approx is None or not self.scanner:
            return
        try:
            ...
            if getattr(self, 'recognition_plugin', None) and self.recognition_plugin:
                try:
            ...
            if not card_info and self.scanner:
            ...
            accepted, seen_count, required_count = self._passes_stability_gate(card_info, scan_origin=scan_origin)
            if not accepted:
                reason = str(card_info.get('fallback_reason') or '').strip() or 'unknown'
                self.detection_info = {
                    'status': 'scanning',
                    'message': f"Stabilizing {seen_count}/{required_count}...",
                    'bin': 'Hold'
                }
                self.log_status(
                    f"~ Stability hold {seen_count}/{required_count}: "
                    f"{card_info.get('name', 'Unknown')} [{reason}]"
                )
                return
            ...
            if self.scanner and self.scanner.track_inventory:
            ...
            fallback_reason = str(card_info.get('fallback_reason') or '').strip()
            review_flag = bool(card_info.get('manual_review_required'))
            
            suffix = ""
            if fallback_reason:
                suffix += f" [{fallback_reason}]"
            if review_flag:
                suffix += " [review]"

            self.log_status(f"✓ {name} ({game}) {confidence:.1f}% → {bin_result}{suffix}")
    ...
    def _draw_detection_info(self, frame, info):
        ...
        if status == 'success':
            ...
            if card.get('market_price') is not None:
                try:
                    price_val = card.get('market_price')
    ...
    def _update_canvas(self, frame):
        ...
        # Keep a reference to prevent GC - store in a dictionary
        if not hasattr(self, '_photo_cache'):
            self._photo_cache = {}
        self._photo_cache['canvas_photo'] = photo
